...
两个应用可同时运行，拥有不同的安装标识、快捷方式、入口、IPC 接口和配置目录。用户版支持多账号窗口：再次启动会打开尚未打开的已有账号数据；已有账号窗口都打开后，再次启动只会唤回现有窗口，不会进入空的数据目录。确实需要新增账号时点击右上角“新建账号窗口”。每个窗口可登录不同成员账号，并分别保存连接、会话、草稿和传输记录；一个窗口切换或断开账号不会影响其他窗口。用户版不包含管理员 Python 程序、用户管理接口或“切换管理员”按钮。
