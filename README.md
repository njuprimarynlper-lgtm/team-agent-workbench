...
只有成员本机无法访问模型服务时才需要此通路：管理端所在电脑必须能访问模型服务或已有 HTTP/SOCKS5 上游代理，并启用“网络出口”。成员取得并粘贴对应管理端的接入码，默认直接连接该电脑提供的地址和 TCP 端口（默认 `18443`）。

成员无法直接连接管理端时，可使用 **经共享服务器中转**：用户端 → 共享服务器 → 管理端 → 模型服务。管理员登录共享服务器，在 **网络出口 → 共享服务器中转** 点击 **启用反向隧道**，再 **复制中转接入码**。成员断开共享连接后重新登录并粘贴此码。管理端主动建立回程连接，复用共享服务器现有 SSH 端口（例如 `2202`），无需新增对外端口，也不要求共享服务器直接连接管理端 IP。短暂断线后自动重连，内部端口保持不变；重启管理端后需重新登录服务器。详见[中转配置步骤](docs/network-egress.md#经共享服务器中转)。
