...
Windows 本地 Agent 工作台与独立的团队管理应用。远端使用 Linux 账号、SSH/SFTP 和文件系统，无常驻业务服务。当前版本 **0.2.1**。
...
- `TeamAgent-admin-0.2.1-x64.exe`：管理员版安装程序。
- `TeamAgent-user-0.2.1-x64.exe`：用户版安装程序。
...
两个应用启动时均打开连接对话框。用户版必须同时填写已存在的 Windows 本机工作路径和 Linux 工作路径；后台使用登录账号实际检查远端目录是否存在、是否为目录、能否进入及打开。验证失败或取消配置时停留在设置页面，主进程也拒绝创建会话等工作操作。每次启动重新连接验证；已开始的本地会话不会因后续网络中断而被终止。地址、端口、Linux 账号、工作路径和服务器指纹可保存；登录密码和 sudo 密码只保留在本次连接的内存中，不落盘，不进入模型 prompt。修改信息后重新连接即可生效。
...
创建时排他地保留项目目录名；同名目录不覆盖。轨迹与成果子目录准备完成、元数据写好后才发布项目。失败时只尝试删除本次创建的空目录，不删除已有项目内容。轨迹和成果的账号目录以 0700 创建，上传无需用户再次填写目标路径。
