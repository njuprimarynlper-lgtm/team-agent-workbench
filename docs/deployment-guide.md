...
3. 创建用户组、成员及子管理员，为各用户启动器预置本地共享区和账号。
...
3. 执行“初始化团队空间”，一次创建受保护的账号登记、成员组、`internal-sftp` 登录规则、受限文件操作器和元数据目录。工具先运行 `sshd -t`，成功后才重载 SSH 服务；失败时保留恢复记录且不开放成员创建。
4. 创建工作组并准备目录。服务器需要 `setfacl`；项目共享读写最终由 Linux 所有权、组和 ACL 执行。
