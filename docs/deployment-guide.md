...
- 启动时检查 Node.js / npm 和已安装依赖，符合当前锁文件时直接复用；实际依赖、安装脚本或已记录的 Node.js 主版本变化，以及缺包或安装中断时才重新安装，日志会说明原因。项目描述、测试命令和文件格式变化不触发重装；每次启动仍会重新构建。两个入口同时打开时会排队准备环境，准备完成后应用可同时运行。
...
以下是把准备步骤合并执行的便捷方式。需要在终端逐步执行安装、检查每步结果时，使用 [README 第 3～5 步](../README.md#project-dependencies)。首次改用双击入口会核对现有 npm 安装记录和依赖版本；手动或离线安装符合当前锁文件时直接复用，不会仅因缺少启动器记录而重装。
