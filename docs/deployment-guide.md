...
- 两个开发入口每次启动都会先执行 `npm run build`，构建成功后才打开应用；构建失败会显示错误并停止启动。拉取代码后关闭旧窗口，再双击对应入口即可运行最新源码，无需手动构建。已打开的窗口不会自动更新。
...
7. 向成员交付服务器地址、SSH 端口、成员账号和初始密码。成员不需要管理员配置文件或服务器指纹。
...
5. 新电脑首次连接时确认一次服务器身份。身份变化时连接会在发送密码前停止；确认服务器确实重装或迁移后，用户在连接设置中重新确认。
