...
2. “本机工作路径”填代码目录；“本地共享区根目录”填管理员提供的测试共享目录（相当于服务器地址）。不填写共享工作路径。
3. 输入成员账号和密码，点击“登录并发现工作组”。左侧按组展示全部授权项目，未分组时提示“还没有加入工作组”，仍可使用本地会话。子管理员在对应组的加号中创建项目；普通成员没有创建入口。成员关系变更后可点击“刷新工作组与项目”，窗口可见时也会每 30 秒自动刷新。
4. 新建会话时选择 Codex。CLI 未登录会显示登录入口；完成个人账号登录后才可发送任务。
5. 左侧浏览共享文件，预览后可加入当前会话。引用保存为本机快照，并记录来源和哈希。
6. 每个会话使用独立的 Agent 工作记录，入口在默认收起的“会话资料”中。AI 提炼成果草稿后由用户审阅、可选补充并确认上传，取消则返回会话并保留草稿，成果包只包含**成果说明、可选 GitHub 仓库链接与元数据**；方向性或结果性结论无需链接即可上传，不包含代码。
7. 会话轨迹默认手动上传，也可为指定会话启用自动上传。其他成员不能经工作台读取你的轨迹。
