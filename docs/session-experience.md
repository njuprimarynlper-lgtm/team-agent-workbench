...
5. AI 自动生成标题、Markdown 成果说明和可选仓库链接，直接展示，无需“采用草稿”。长说明可展开。上传位置自动识别并只读展示；用户只需查看、可选补充、点击“确认上传”。仓库链接可留空，不影响上传方向性判断、结果性结论或方案建议；可选链接入口默认收起，涉及代码时可以补充或更正。人的补充和链接更正独立保存，失败、停止、重试不会覆盖。失败立即显示原因与重试入口，最长等待十分钟后显示超时并停止本次整理。失败重试使用原冻结材料；完成后“重新整理”读取当前最新交接材料。应用退出不在后台继续整理，重开显示已中断并保留内容。
