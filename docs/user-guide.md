...
地址、端口、账号和已指定的代码目录会记住；密码不保存，每次启动仍需输入。修改连接时，点击右上角服务器按钮，直接修改相应字段。
...
| 位置或入口 | 用途 |
| --- | --- |
| 最左侧功能栏 | 切换任务、动态、会话、本地项目成果库、团队项目成果库、成果整理、传输记录等页面；鼠标停留可查看名称 |
...
1. 选择项目，点击新建会话入口。
2. 选择 **Codex**、**Cursor** 或 **Claude Code**，确认**代码目录（选填）**、模型和执行权限。留空适合调研，会话使用独立的持久目录；已有会话目录不会因此迁移或删除。
...
每个账号可在**设置 → 我的成果分类**为当前项目选择一个分类组合，也可从整理窗口点击**调整我的分类**。例如，做软件开发时可以选包含“需求说明、设计方案、验证结果、问题与风险”的组合；本次整理还可临时取消不需要的类别。更改组合只影响后续整理，不会给旧成果重新分类。
...
左侧**本地项目成果库**汇集从团队成果采用、会话整理、手工新建或已有成果再处理形成的个人内容，按当前账号和项目显示，本机保留离线缓存并随账号同步。它与组管理员维护的公共**项目说明**、供团队阅读的**团队项目成果库**不同；在本地编辑不会修改团队共享内容。
...
具体可用类别由当前项目的**我的成果分类**决定，12 类的适用场景见[核对类别与内容](#section-6-2)。旧成果仍按原类别显示。两个成果即使标题相同，只要来源不同就分别保留；同一来源更新时，手工修改过的正文不会被新版本覆盖。
...
选择项目后进入**设置 → 我的成果分类**，为这个项目选择算法研究、软件开发、调研分析等预设，或新建自己的分类组合。创建组合时从已有类别中选择，不必重新发明类别和生成规则。整理范围窗口也有**调整我的分类**入口。
...
点击回复中的本地文件链接，或打开**会话文件**。文本和图片可预览，PDF、Office 等可使用系统应用打开，也可复制路径或打开所在文件夹。
...
入口：用户版 → 选择项目 → **团队项目成果库**。普通成员也从这里阅读团队成果；组管理员在同一页面有额外的编辑、合并和删除操作。

默认展示**未操作**内容，可通过**团队成果操作状态**切换到已操作。保存整理、完成合并或纳入管理后进入已操作；仅查看、设置别名或加入会话不改变状态。
...
**批量删除：**点击**批量删除团队成果**，在当前项目、分组和筛选结果中选择要删除的成果、文件或轨迹。全选最多 100 项。打开确认框，逐项核对名称、提交人和版本，再确认删除。

只能删除自己有权限处理的内容。普通成员仅可选择自己上传、且尚未由组管理员整理的内容。切换项目、分组、搜索条件、类型或刷新页面时，已选项目会清空；批量删除与语义合并不能同时进行。
...
**类别与预期不同：**检查**设置 → 我的成果分类**中当前项目的组合，上传前核对并更正类别。修改组合只影响后续整理，不会重写旧成果。
