...
async function main() {
  ...
  try {
    ...
    await page.getByTitle('团队动态', { exact: true }).click();
    await entry.getByRole('button', { name: '存为项目笔记', exact: true }).click();
    ...
    await expect(entry.getByLabel('我的处理记录')).toContainText('已存为项目笔记：“接口超时结论”');
    await expect(entry).toContainText('已处理');
    await entry.getByRole('button', { name: '存为项目笔记', exact: true }).click();
    await expect(entry.getByLabel('我的处理记录').locator('li')).toHaveCount(1);

    await page.getByTitle('我的项目笔记', { exact: true }).click(); await page.locator('.content-card-summary').click();
    const setAlias = async (name: string) => {
      await page!.getByRole('button', { name: '设置本地别名', exact: true }).click();
      await page!.getByLabel('笔记本地别名').fill(name); await page!.getByRole('button', { name: '保存本地别名', exact: true }).click();
      await expect(page!.getByRole('dialog', { name: '设置笔记别名' })).toHaveCount(0);
    };
    ...
    await expect(page.getByRole('dialog', { name: '选择使用笔记的会话' })).toContainText('接口超时验收约束');
    await page.getByLabel('使用笔记的会话：新会话').check(); await page.getByRole('button', { name: '加入 1 个会话', exact: true }).click();
    await expect(page.getByRole('dialog', { name: '选择使用笔记的会话' })).toHaveCount(0);
    ...
    await removed.getByRole('button', { name: '选择是否保留项目笔记', exact: true }).click();
    const decision = page.getByRole('dialog', { name: '是否保留项目笔记？' });
    await expect(decision.getByLabel('删除项目笔记：接口超时验收约束')).not.toBeChecked();
    await expect(decision.getByRole('button', { name: '删除选中的 0 条项目笔记', exact: true })).toBeDisabled();
    await page.screenshot({ path: path.join(data, 'local-delete-choice.png') });
    await decision.getByRole('button', { name: '保留全部项目笔记', exact: true }).click();
    ...
    await page.getByTitle('我的项目笔记', { exact: true }).click(); await expect(page.locator('.conclusion-library')).toContainText('接口超时验收约束');
    await page.getByTitle('团队动态', { exact: true }).click(); await page.getByRole('tab', { name: /全部动态/ }).click();
    ...
    const deletionEntry = page.locator('.update-entry').filter({ has: page.getByRole('button', { name: '选择是否保留项目笔记', exact: true }) });
    await expect(deletionEntry.getByLabel('我的处理记录')).toContainText('已保留项目笔记：“接口超时验收约束”');
    await deletionEntry.getByRole('button', { name: '选择是否保留项目笔记', exact: true }).click();
    await page.getByLabel('删除项目笔记：接口超时验收约束').check(); await page.getByRole('button', { name: '删除选中的 1 条项目笔记', exact: true }).click();
    await expect(page.getByRole('dialog', { name: '是否保留项目笔记？' })).toHaveCount(0);
    ...
    await expect(deletionEntry.getByLabel('我的处理记录')).toContainText('已删除项目笔记：“接口超时验收约束”');
    ...
    await page.getByTitle('我的项目笔记', { exact: true }).click();
    await expect(page.locator('.content-card')).toHaveCount(2);
    await page.getByRole('button', { name: '批量删除笔记', exact: true }).click();
    await expect(page.getByRole('button', { name: '删除选中的 0 条笔记', exact: true })).toBeDisabled();
    await page.getByLabel('全选当前笔记', { exact: true }).check();
    await page.getByRole('button', { name: '显示历史笔记', exact: true }).click();
    await expect(page.locator('.content-card')).toHaveCount(3);
    await expect(page.getByLabel('全选当前笔记', { exact: true })).not.toBeChecked();
    await page.getByRole('button', { name: '隐藏历史笔记', exact: true }).click();
    await page.getByLabel('全选当前笔记', { exact: true }).check();
    await page.getByRole('button', { name: '删除选中的 2 条笔记', exact: true }).click();
    let confirmLocal = page.getByRole('dialog', { name: '删除项目笔记？', exact: true });
    ...
    await confirmLocal.getByRole('button', { name: '确认删除 2 条项目笔记', exact: true }).click();
    ...
    confirmLocal = page.getByRole('dialog', { name: '删除项目笔记？', exact: true });
    await confirmLocal.getByRole('button', { name: '确认删除项目笔记', exact: true }).click();
