...
try {
  ...
  for (const label of ['nlp', 'ocr']) await expect(page.locator('.modal .check-row').filter({ hasText: label + ' · 内容组管理员' }).locator('input')).toBeChecked();
  await page.locator('.modal .check-row').filter({ hasText: 'ocr · 内容组管理员' }).locator('input').uncheck(); await confirm();
  ...
  await expect(group('nlp').locator('[data-user="alice"]')).toContainText('本组组管理员');
  ...
  await group('nlp').locator('[data-user="bob"]').getByRole('button', { name: '设为组管理员', exact: true }).click();
  await page.getByRole('button', { name: '取消', exact: true }).click();
  ...
  await group('ocr').locator('[data-user="solo"]').getByRole('button', { name: '设为组管理员', exact: true }).click(); await confirm();
  ...
  await page.getByRole('tab', { name: '按组查看', exact: true }).click();
  await group('nlp').locator('[data-user="bob"]').getByRole('button', { name: '设为组管理员', exact: true }).click(); await confirm();
  await expect(group('nlp').locator('[data-user="bob"]')).toContainText('本组组管理员');
  await group('nlp').locator('[data-user="bob"]').getByRole('button', { name: '取消组管理员', exact: true }).click(); await confirm();
