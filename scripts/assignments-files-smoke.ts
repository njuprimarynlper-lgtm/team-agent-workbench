...
async function main() {
  ...
  try {
    ...
    await a.page.getByRole('button', { name: '项目任务', exact: true }).click();
    ...
    await form.getByLabel('关联成果：【项目结论】 OCR 回归基线', { exact: true }).check();
    ...
    await b.page.getByRole('button', { name: '我的任务', exact: true }).click(); await b.page.getByRole('button', { name: '提交验收', exact: true }).click();
    const review = b.page.getByRole('dialog', { name: '提交验收？' });
    await review.getByLabel('任务结果说明', { exact: true }).fill('已提供回归样本和验证报告。');
    await review.getByRole('button', { name: '提交验收', exact: true }).click(); await expect(review).toHaveCount(0);
    await a.page.getByRole('button', { name: '刷新任务', exact: true }).click();
    await a.page.getByRole('button', { name: '验收通过', exact: true }).click();
    await a.page.getByRole('dialog', { name: '确认完成？' }).getByRole('button', { name: '确认完成', exact: true }).click();
    await a.page.getByRole('tab', { name: /全部状态/ }).click();
