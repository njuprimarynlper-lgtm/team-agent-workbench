...
try {
  ...
  await expect(page.getByLabel('代码目录（选填）', { exact: true })).toHaveValue('');
  ...
  await expect(page.getByLabel('代码目录（选填）', { exact: true })).toHaveValue(workspace);
  await expect(page.getByRole('button', { name: '登录', exact: true })).toBeEnabled();
  ...
  await expect(page.getByLabel('代码目录（选填）', { exact: true })).toHaveValue('');
  await page.getByLabel('代码目录（选填）', { exact: true }).fill(workspace);
  ...
  console.log(JSON.stringify({ passed: true, data, cases: ['fresh start shows a direct login form', 'admin egress is visibly optional and only asks for an invite code when enabled', 'local fixture hides shared root', 'optional code directory can be selected', 'legacy multiple profiles never add a connection selector', 'server and account change in the same form', 'incomplete local fixture is rejected', 'server identity details stay hidden', 'restart refills the latest server and account ahead of an older verified snapshot, never password or unsaved edits'] }));
} catch (e) {
