...
try {
  ...
  await expect.poll(async () => (await snap()).sessions.find(s => s.id === first.id).status).toBe('idle');
  await expect(page.getByLabel('当前执行权限')).toContainText('请求批准');
  ...
  await expect(page.getByRole('dialog', { name: '执行权限', exact: true })).not.toContainText(/1385|CreateProcess|查看错误详情|查看检测详情/);
  await page.setViewportSize({ width: 1100, height: 760 });
  const apply = page.getByRole('button', { name: '完全访问', exact: true });
  ...
  await expect.poll(async () => (await snap()).sessions.find(s => s.id === first.id).status).toBe('idle');
  await expect(page.getByLabel('当前执行权限')).toContainText('完全访问');
  ...
  await expect(page.getByLabel('当前执行权限')).toContainText('Allowlist（白名单）');
  ...
  await expect(page.getByLabel('当前执行权限')).toContainText('自定义设置（请求批准）');
  await expect(page.getByLabel('当前执行权限')).toHaveAttribute('title', /目前仅允许读取，修改文件或运行写入命令需要额外授权/);
  await expect(page.getByLabel('当前执行权限')).not.toContainText(/sandbox|CreateProcess|自检/);
  await page.getByLabel('当前执行权限').click();
  await expect(page.getByRole('dialog', { name: '执行权限', exact: true })).not.toContainText(/CreateProcess|查看错误详情|查看检测详情/);
  await page.getByRole('button', { name: '关闭选项', exact: true }).click();
