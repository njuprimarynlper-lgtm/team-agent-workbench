...
try {
  ...
  for (const seed of seeds) {
    ...
    await page.getByLabel('带入笔记：OCR 识别约束', { exact: true }).check();
  ...
  await page.getByLabel('选择笔记：单条启动约束', { exact: true }).check();
  await expect(page.getByRole('button', { name: '处理选中的 1 条笔记', exact: true })).toBeEnabled();
  await page.getByRole('button', { name: '显示历史笔记', exact: true }).click();
  await expect(page.getByRole('button', { name: /处理选中的/ })).toHaveCount(0);
  await page.getByLabel('选择笔记：单条启动约束', { exact: true }).check();
  await page.getByRole('button', { name: '处理选中的 1 条笔记', exact: true }).click();
  const processDialog = page.getByRole('dialog', { name: '处理所选笔记', exact: true });
  await expect(processDialog).toContainText('所选原笔记才会移入历史');
  ...
  await page.getByLabel('笔记处理要求').fill(direction);
  await page.getByLabel('用于处理笔记的会话').selectOption(seeds[0].id);
  ...
  await page.getByRole('button', { name: '保存新笔记并将 1 条原笔记移入历史', exact: true }).click();
