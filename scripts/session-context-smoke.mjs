...
try {
  ...
  await page.getByLabel('选择结论：单条启动约束', { exact: true }).check();
  await expect(page.getByRole('button', { name: '处理选中的 1 条结论', exact: true })).toBeEnabled();
  await page.getByRole('button', { name: '显示历史结论', exact: true }).click();
  await expect(page.getByRole('button', { name: /处理选中的/ })).toHaveCount(0);
  await page.getByLabel('选择结论：单条启动约束', { exact: true }).check();
  await page.getByRole('button', { name: '处理选中的 1 条结论', exact: true }).click();
  const processDialog = page.getByRole('dialog', { name: '处理所选结论', exact: true });
  await expect(processDialog).toContainText('所选原结论才会移入历史');
  ...
  await page.getByLabel('结论处理要求').fill(direction);
  await page.getByLabel('用于处理结论的会话').selectOption(seeds[0].id);
  ...
  await page.getByRole('button', { name: '保存新结论并将 1 条原结论移入历史', exact: true }).click();
