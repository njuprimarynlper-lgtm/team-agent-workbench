...
await fs.appendFile(path.join(demo, '开发版同步.md'), '\n会话输入框底部可直接选择模型和权限，模型菜单内可查看账号额度。切换保留已有对话与未发送草稿；正在运行时需确认停止当前任务。顶部压缩为紧凑工具栏，项目说明和阶段摘要收在“AI 参考内容”菜单内。\n', 'utf8');
await fs.appendFile(path.join(demo, '开发版同步.md'), '\n成果整理默认使用完全访问；新建和重新整理均不继承来源会话的人工审批配置。已有等待审批的整理可取消后重新整理。上传仍由用户确认。\n', 'utf8');
const guide = path.join(demo, '使用说明.md');
await fs.appendFile(path.join(demo, '开发版同步.md'), '\n新增空组首次使用引导：首位新建或添加的成员默认组管理员（可调整）。组管理员登录空组时填写项目背景、目标、验收标准等内容，创建首个项目和《项目说明.md》，也可稍后填写并保留草稿。现有项目及成员身份不自动改变。\n', 'utf8');
await fs.appendFile(path.join(demo, '开发版同步.md'), '\n用户版只选择本机工作路径，共享区根目录由联调启动器预置，不再要求用户选择。正式 SSH 连接直接填写服务器地址、端口、账号和密码，不导入管理员配置。服务器身份首次确认后保存在当前 Windows 账号下，不要求用户填写指纹；身份变化时可主动重新确认。只有一份连接时直接显示连接说明，多个连接才显示有标签的选择器；工作组在一次登录后自动发现，不需要每组建立连接。\n', 'utf8');
const original = await fs.readFile(guide, 'utf8').catch(e => { if (e.code === 'ENOENT') return ''; throw e; });
if (original) {
  const note = `> 当前三个启动文件使用最新开发构建，程序位于 \`${path.join(repo, 'dist')}\`，不再依赖旧版安装包。共享目录、账号及历史会话继续使用本目录原有数据。本地管理员无需账号密码，启动自动打开已保存的共享目录；“创建用户 / 创建用户组”已并排；用户版的整理成果已改为后台等待面板、AI 自动说明与上传位置、小型可选补充、人工确认上传；方向性与结果性结论无需仓库链接即可上传，链接为可选项；阶段摘要位于默认收起的“AI 参考内容”中，先只读预览；整理结果页采用单列阅读流，返回会话不停止后台整理，停止整理使用独立按钮；用户登录后自动发现全部工作组，按组展示项目，无需填写共享工作路径；正式 SSH 连接直接填写地址、端口、账号和密码，首次确认的服务器身份保存在当前 Windows 账号下，不要求导入管理员配置或填写指纹；未分组账号可登录但没有工作台，界面提示联系管理员；组权限变更后刷新即可；会话固定绑定原项目。新建会话可选择 CLI 原有设置、人工审批或完全权限，现有会话可检测与修改权限，命令受限和待授权操作会明确提醒；修改权限不会自动重试任务。公共成果支持检索、引用、本人修订和组管理员整理/合并/锁定；项目资料有版本与更新提示；整理可附代码版本；轨迹上传去重，可清理成功缓存。模型选择和轨迹上传继续保留。重新打开启动文件即可查看；以下四轮比赛记录属于历史验证。`;
