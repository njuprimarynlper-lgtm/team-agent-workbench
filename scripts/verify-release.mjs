...
for (const edition of ['admin', 'user']) {
  const base = path.join('release', edition, 'win-unpacked'), archive = path.join(base, 'resources/app.asar');
  assert.equal(JSON.parse(extractFile(archive, 'package.json').toString()).version, version);
  for (const file of ['main.cjs', 'preload.cjs', 'renderer.js', 'index.html']) assert.equal(sha(extractFile(archive, file)), sha(await fs.readFile(path.join('dist', edition, file))), edition + '/' + file);
  ...
  for (const ext of ['exe', 'zip']) {
    const file = path.join('release', edition, `TeamAgent-${edition}-${version}-x64.${ext}`);
...
for (const [provider, source] of [['cursor', '.tools/cursor/dist-package'], ['codex', 'node_modules/@openai/codex-win32-x64/vendor/x86_64-pc-windows-msvc']]) {
  ...
  for (const file of files) assert.equal(sha(await fs.readFile(path.join(source, file))), sha(await fs.readFile(path.join('release/user/win-unpacked/resources/providers', provider, file))), provider + '/' + file);
  console.log(provider, files.length, 'runtime files verified');
}
const contents = hashes.join('\n') + '\n';
await fs.writeFile(`release/SHA256SUMS-${version}.txt`, contents);
