...
export function EgressMonitorPanel({ snapshot }: { snapshot: AdminEgressSnapshot }) {
  ...
  const history = monitor?.history || [], max = Math.max(1, ...history.flatMap(point => [point.upPerSecond, point.downPerSecond]));
  const points = (field: 'upPerSecond' | 'downPerSecond') => history.map((point, i) => `${i * 600 / Math.max(1, history.length - 1)},${76 - point[field] / max * 68}`).join(' ');
  ...
  return <section className="egress-monitor" aria-label="代理独立监控">
    ...
    <div className="relay-trend"><div className="relay-trend-label"><span><i className="up"/>上行 <i className="down"/>下行 · 最近 2 分钟</span><small>峰值 {traffic(max === 1 && !history.some(point => point.upPerSecond || point.downPerSecond) ? 0 : max)}/s</small></div>
      <svg viewBox="0 0 600 84" preserveAspectRatio="none" role="img" aria-label="代理上下行速率趋势"><path d="M0 76H600 M0 42H600 M0 8H600" className="grid"/><polyline points={points('upPerSecond')} className="up"/><polyline points={points('downPerSecond')} className="down"/></svg>
