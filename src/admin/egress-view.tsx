...
export function EgressView({ snapshot, refresh }: { snapshot: AdminEgressSnapshot; refresh: () => Promise<void> }) {
  ...
  const run = async (fn: () => Promise<void>) => { setBusy(true); setError(''); setMessage(''); try { await fn(); } catch (e: any) { setError(e.message); } finally { setBusy(false); } };
  const save = () => run(async () => { await api.call('egress.save', { config, upstreamPassword: password, clearUpstreamPassword: clearPassword }); setPassword(''); setClearPassword(false); setTestResults({}); await refresh(); setMessage(config.enabled ? '网络出口已保存并启动' : '网络出口已关闭，用户端可继续使用本机直连'); });
  const probe = async (provider: EgressService) => {
    setBusy(true); setTesting(provider); setError(''); setMessage(''); setTestResults(current => ({ ...current, [provider]: undefined }));
    try { await api.call('egress.test', { provider }); setTestResults(current => ({ ...current, [provider]: { ok: true, detail: '管理端网络连接通过' } })); }
    catch (error: any) { setTestResults(current => ({ ...current, [provider]: { ok: false, detail: error.message || '网络连接失败' } })); }
    finally { setBusy(false); setTesting(undefined); }
  };
  return <div className="egress-view">
    ...
    <section className="egress-card"><header><div><h3>出口设置</h3><small>关闭后不会影响用户端本地工作和共享空间。</small></div><label className="switch-row"><input type="checkbox" checked={config.enabled} onChange={e => update('enabled', e.target.checked)}/>启用管理端出口</label></header><div className="egress-form">
      <div className="form-grid"><label className="field">提供给成员的地址<input aria-label="提供给成员的地址" value={config.publicHost} onChange={e => update('publicHost', e.target.value)}/><small>自动填入本机名；无法解析时改成固定内网 IP。</small></label><label className="field">监听端口<input aria-label="网络出口端口" type="number" value={config.listenPort} onChange={e => update('listenPort', Number(e.target.value))}/></label></div>
      <div className="egress-service-permissions"><span>成员可通过出口访问</span><div>{egressServices.map(service => <label className="check-row" key={service.id}><input type="checkbox" checked={config[service.id]} onChange={e => update(service.id, e.target.checked)}/>{service.label}</label>)}</div><small>只控制成员转发权限，无需在管理端安装这些 CLI。修改后点击“保存并应用”。</small></div>
      <details><summary>高级监听设置</summary><div className="egress-details"><label className="field">本机监听地址<input value={config.listenHost} onChange={e => update('listenHost', e.target.value)}/></label></div></details>
