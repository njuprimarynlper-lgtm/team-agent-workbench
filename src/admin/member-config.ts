...
export function memberConfig(profile: AdminProfile, state: AdminState, username: string, groupName: string) {
  ...
  const id = profile.mode === 'local' ? 'local_' + createHash('sha256').update(JSON.stringify([state.teamId, username, group.name])).digest('hex').slice(0, 40) : `${state.teamId}-${username}-${group.label}`;
