...
export const projectSchema = z.object({ id: z.string().regex(/^[a-zA-Z0-9_-]{1,80}$/), name: z.string().min(1).max(120), remoteRoot: absoluteRemote, uploadPath: absoluteRemote, historyPath: absoluteRemote, managed: z.boolean().optional() }).refine(p => withinRemote(p.remoteRoot, p.uploadPath) && withinRemote(p.remoteRoot, p.historyPath), '上传与历史目录必须位于项目根路径内');
...
export const settingsSchema = z.object({ connections: z.array(profileSchema).max(50), providerPaths: z.object({ codex: z.string(), cursor: z.string() }), lastWorkspace: z.string(), localWorkspace: z.string().default('') });
