...
export const userEgressSettingsSchema: z.ZodType<UserEgressSettings> = z.object({
  enabled: z.boolean().default(false), host, port, certificateFingerprint: fingerprintSchema,
});

const inviteSchema: z.ZodType<EgressInvite> = z.object({ version: z.literal(1), host, port, fingerprint: fingerprintSchema, accessCode: z.string().min(24).max(512) });

export function encodeEgressInvite(invite: EgressInvite) {
  return 'TAE1.' + Buffer.from(JSON.stringify(invite), 'utf8').toString('base64url');
}

export function decodeEgressInvite(value: string): EgressInvite {
  const clean = value.trim();
  if (!clean.startsWith('TAE1.') || clean.length > 4096) throw new Error('接入码格式无效');
