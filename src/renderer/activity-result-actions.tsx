...
export function ActivityResultActions({ event, item, disabled = false, changed, notice }: {
  ...
  const run = async (action: 'save' | 'status') => {
    ...
    try {
      if (action === 'save' && item) {
        ...
        const label = ({ created: '已存入个人成果库', updated: '已更新本地成果来源', duplicate: '这条成果已在个人成果库中' } as const)[result.action];
  ...
  return <section className="activity-result-actions" aria-label="处理这条动态">
    ...
    <div className="row">
      {item && <button className="primary" disabled={disabled || !!busy} onClick={() => void run('save')}><Download size={15}/>{busy === 'save' ? '正在存入…' : '存入个人成果库'}</button>}
