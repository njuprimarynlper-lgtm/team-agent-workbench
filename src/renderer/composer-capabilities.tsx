  ...
  return <div className="composer-capabilities" ref={root}>
    ...
    {open && <section className="composer-popover capability-popover" role="dialog" aria-label="Skill 与插件">
      ...
      <div className="composer-options">
        <p className="capability-help">选择项只用于下一条消息。{session.provider === 'codex' ? 'Codex 按原生 $Skill / @插件提及调用。' : 'Cursor Skill 按原生 / 命令调用；插件工具由 CLI 配置加载。'}</p>
        ...
        <CapabilitySection title={session.provider === 'codex' ? '已安装插件' : '插件工具（MCP）'} label="可用插件" items={plugins} selected={selected} loading={loading} empty="没有可选择的插件工具。" toggle={toggle}/>
      </div>
    </section>}
  </div>;
}
