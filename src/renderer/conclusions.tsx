...
import { materialCategories, contributionCategoryInfo, contributionTitle, titleSubject, type ContributionCategory } from '../shared/content';

const api = window.workbench;
const conclusionTitle = (value: ProjectConclusion) => resultLabelTitle(value.title, value.titleAlias);
...
export function ConclusionLibrary({ project, sessions, notice, mergeStarted, focusId, focusHandled, refreshToken, categories = [...materialCategories], embedded = false }: { embedded?: boolean; categories?: ContributionCategory[]; refreshToken?: string; project: Project; sessions: AgentSession[]; notice: (text: string) => void; mergeStarted: (draft: Draft) => void; focusId?: string; focusHandled?: () => void }) {
  ...
  const [category, setCategory] = useState<ContributionCategory>('finding');
  const [categoryFilter, setCategoryFilter] = useState('all'), [query, setQuery] = useState('');
  ...
  useEffect(() => { setItems([]); setSelected(''); setCreating(false); setEditing(false); setCategoryFilter('all'); setQuery(''); setShowArchived(false); setAliasItem(undefined); setPendingDelete([]); setSelectingDelete(false); setDeleteSelection([]); setMergeSelection([]); setConfirmingMerge(false); setInstruction(''); void load(); }, [project.id]);
  ...
  const visible = scopedItems.filter(value => matchesResultLabel(value.title, categoryFilter) && `${conclusionTitle(value)} ${value.title} ${value.content}`.toLocaleLowerCase().includes(query.toLocaleLowerCase()));
  ...
  const open = (value: ProjectConclusion) => { setSelected(value.id); setCreating(false); setEditing(false); setTitle(titleSubject(value.title)); setCategory(value.category || materialCategories.find(key => value.title.startsWith(`【${contributionCategoryInfo[key].label}】`)) || 'finding'); setContent(value.content); };
  const beginCreate = () => { setSelected(''); setCreating(true); setEditing(true); setTitle(''); setCategory(categories.includes('finding') ? 'finding' : categories[0]); setContent(''); setCategoryFilter('all'); setQuery(''); };
  const save = async () => {
    if (!title.trim() || !content.trim()) return; setBusy(true); setError('');
    try { const saved = creating ? await api.call<ProjectConclusion>('conclusion.create', { projectId: project.id, title: contributionTitle(category, title), content, category }) : await api.call<ProjectConclusion>('conclusion.save', { id: item!.id, title: contributionTitle(category, title), content, category }); setCreating(false); setEditing(false); setCategoryFilter('all'); setQuery(''); await load(); setSelected(saved.id); notice(creating ? '项目成果已创建' : '项目成果已保存为新版本'); }
    catch (e: any) { setError(e.message); } finally { setBusy(false); }
  };
  ...
  const renderEditor = () => <>
    <label className="field">类别<select aria-label="成果类别" value={category} onChange={event => setCategory(event.target.value as ContributionCategory)}>{[...new Set([...categories, ...(!creating && item ? [item.category || category] : [])])].map(key => <option key={key} value={key}>{contributionCategoryInfo[key].label}{!categories.includes(key) ? '（原类别）' : ''}</option>)}</select></label>
  ...
  const renderActions = (value: ProjectConclusion) => {
    ...
    return <>
      ...
      {value.archived ? <button className="secondary compact" disabled={disabled} onClick={() => void archive(value, false)}><RotateCcw size={14}/>恢复使用</button> : <button className="secondary compact" disabled={disabled} title="暂时停用，保留内容，可随时恢复" onClick={() => void archive(value, true)}><Archive size={14}/>移入历史</button>}
  ...
  return <div className={(embedded ? 'results-library-pane' : 'workspace-page') + ' conclusion-page'}>
    ...
    <div className="content-library result-library conclusion-library"><div className="content-cards">
      ...
      {visible.map(value => <ResultCard key={value.id} id={value.id} title={conclusionTitle(value)}
        badges={<><span className="content-category-badge">{resultLabels(value.title).join(' · ') || '未分类'}</span>{value.archived && <span className="result-history-badge">历史成果</span>}</>}
        metadata={<>{sourceSummary(value)}{value.sources.length > 1 ? ` · ${value.sources.length} 个来源` : ''} · {new Date(value.updatedAt).toLocaleString()}</>}
