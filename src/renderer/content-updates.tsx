...
const changeInfo = {
  ...
  deleted: { label: '共享区已移除', detail: '这条内容已从共享区移除，已保存的项目笔记不会自动删除', icon: Trash2 },
...
function actionLabel(action: ContentUpdateAction, event: ContentUpdate) {
  ...
  return ({ saved_conclusion: '已存为项目笔记', attached_session: '已加入会话', kept_conclusion: '已保留项目笔记', deleted_conclusion: '已删除项目笔记', acknowledged: '已确认，没有对应项目笔记', archived: '已标记处理' } as const)[action.kind] + name;
}
...
export function ContentActionRecord({ action, event }: { action: ContentUpdateAction; event: ContentUpdate }) {
  ...
  return <><span>{actionLabel(action, event)}{action.sourceRevision !== undefined && <small> · 第 {action.sourceRevision} 版{newer ? `（此动态记录的是第 ${event.revision} 版）` : ''}</small>}
    ...
    {action.correctedFromTitle && <details><summary>已纠正旧版自动归并</summary>已恢复为独立笔记。原资料“{action.correctedFromTitle}”的正文和会话引用保持不变。</details>}
...
export function UpdatedContentConfirmation({ original, latest, busy, confirm, close, view }: { original: ContentUpdate; latest: ContentUpdate; busy: boolean; confirm: () => void; close: () => void; view: () => void }) {
  return <div className="modal-backdrop"><section className="modal" role="dialog" aria-modal="true" aria-labelledby="updated-content-title">
    ...
    <footer><button className="secondary" disabled={busy} onClick={close}>取消</button><button className="secondary" disabled={busy} onClick={view}>查看更新后的结果</button><button className="primary" disabled={busy} onClick={confirm}>将第 {latest.revision} 版存为项目笔记</button></footer>
  </section></div>;
}
...
export function ContentUpdatesPanel({ updates, aliases, apply, view, deletionResolved }: { updates: ContentUpdate[]; aliases: Record<string, string>; apply: (item: ContentUpdate) => Promise<ConclusionOrganization | undefined>; view: (item: ContentUpdate) => void; deletionResolved: () => Promise<void> }) {
  ...
  return <div className="workspace-page updates-page">
    <div className="page-title updates-heading"><div><span className="eyebrow">TEAM ACTIVITY</span><h1>团队动态</h1><p className="muted small">这里记录公共区发生过的变化，包括自己的操作；队友的变化和需要处理个人副本的删除进入待处理，其他自己的操作进入历史。“查看结果”打开这条成果的当前版本。“存为项目笔记”保存到你的项目笔记，不会发布到团队共享区。共享区删除后，项目笔记仍会保留，由你选择是否删除个人副本。</p></div></div>
    ...
    {pendingDelete.length > 0 && <div className="modal-backdrop"><section className="modal" role="dialog" aria-modal="true" aria-labelledby="delete-events-title">
      ...
      <div className="modal-body"><p>将从本机删除以下 {pendingDelete.length} 条动态记录。</p><ul className="delete-selection-list">{pendingDelete.map(item => <li key={item.eventId}>{eventTitle(item)} · {changeInfo[item.change].label} · v{item.revision} · {item.projectName}</li>)}</ul><p className="muted small">只清除动态记录，共享区内容、项目笔记和会话引用会继续保留。后续有新的内容变化时，仍会收到新动态。</p>{error && <div className="inline-error" role="alert">{error}</div>}</div>
    ...
    {deletion && <div className="modal-backdrop"><section className="modal" role="dialog" aria-modal="true" aria-labelledby="local-deletion-title">
      <header><h2 id="local-deletion-title">是否保留项目笔记？</h2><button className="icon" aria-label="关闭窗口" disabled={!!busy} onClick={() => setDeletion(undefined)}>×</button></header>
      <div className="modal-body"><p>已从共享区移除：{aliases[contentAliasKey(deletion.event.projectId, deletion.event.id)] || titleSubject(deletion.event.title) || deletion.event.title}</p>
        ...
        {deletion.conclusions.length ? <><p className="muted small">这些项目笔记目前仍然保留。只会删除你明确勾选的条目；包含多个来源的笔记会整条删除。已加入会话的快照会继续保留。</p>
          <div className="conclusion-match-list">{deletion.conclusions.map(item => <div className="local-deletion-option" key={item.id}><label className="check-row"><input type="checkbox" aria-label={`删除项目笔记：${conclusionTitle(item)}`} checked={removeIds.includes(item.id)} disabled={!!busy} onChange={event => setRemoveIds(current => event.target.checked ? [...current, item.id] : current.filter(id => id !== item.id))}/><span><b>{conclusionTitle(item)}</b><small>v{item.version} · {item.sources.length} 个来源{item.sources.length > 1 ? ' · 含其他来源，请确认后选择' : ''}{item.archived ? ' · 历史笔记' : ''}</small></span></label><details><summary>查看内容与来源</summary><p className="local-deletion-content">{item.content}</p>{item.sources.map((source, index) => <p className="muted small" key={index}>{source.title}</p>)}</details></div>)}</div>
        </> : <p>本地没有保存这条成果对应的结论。</p>}
      </div><footer><button className="secondary" disabled={!!busy} onClick={() => setDeletion(undefined)}>稍后处理</button><button className="primary" disabled={!!busy} onClick={() => void resolveDeletion(false)}><Archive size={13}/>{deletion.conclusions.length ? '保留全部项目笔记' : '知道了'}</button>{deletion.conclusions.length > 0 && <button className="secondary danger" disabled={!!busy || !removeIds.length} onClick={() => void resolveDeletion(true)}>删除选中的 {removeIds.length} 条项目笔记</button>}</footer>
    </section></div>}
  </div>;
}
