...
function actionLabel(action: ContentUpdateAction, event: ContentUpdate) {
  ...
  return ({ saved_conclusion: '已存入本地成果库', attached_session: '已加入会话', kept_conclusion: '已保留项目成果', deleted_conclusion: '已删除项目成果', acknowledged: '已确认，没有对应项目成果', archived: '已标记处理' } as const)[action.kind] + name;
}
...
export function UpdatedContentConfirmation({ original, latest, busy, confirm, close, view }: { original: ContentUpdate; latest: ContentUpdate; busy: boolean; confirm: () => void; close: () => void; view: () => void }) {
  return <div className="modal-backdrop"><section className="modal" role="dialog" aria-modal="true" aria-labelledby="updated-content-title">
    ...
    <footer><button className="secondary" disabled={busy} onClick={close}>取消</button><button className="secondary" disabled={busy} onClick={view}>查看更新后的结果</button><button className="primary" disabled={busy} onClick={confirm}>将第 {latest.revision} 版存入本地成果库</button></footer>
  </section></div>;
}
...
export function ContentUpdatesPanel({ updates, aliases, apply, view, deletionResolved }: { updates: ContentUpdate[]; aliases: Record<string, string>; apply: (item: ContentUpdate) => Promise<ConclusionOrganization | undefined>; view: (item: ContentUpdate) => void; deletionResolved: () => Promise<void> }) {
  ...
  return <div className="workspace-page updates-page">
    <div className="page-title updates-heading"><div><span className="eyebrow">TEAM ACTIVITY</span><h1>团队动态</h1><p className="muted small">这里记录公共区发生过的变化，包括自己的操作；队友的变化和需要处理个人副本的删除进入待处理，其他自己的操作进入历史。“查看结果”打开这条成果的当前版本。“存入本地成果库”保存在“本地项目成果库”，不会发布到团队共享区。共享区删除后，项目成果仍会保留，由你选择是否删除个人副本。</p></div></div>
