...
export function DraftEditor({ draft, session, sourceTitle, sourceSession, transfer, run, notice, close }: { draft: Draft; session?: AgentSession; sourceTitle?: string; sourceSession?: AgentSession; transfer?: Transfer; run: <T>(fn: () => Promise<T>) => Promise<T | undefined>; notice: (s: string) => void; close: () => void }) {
  ...
  const cancel = () => void run(async () => {
    ...
    try {
      ...
      await editor.flush();
      if (!draft.submitted && !submitted) notice('草稿已保留');
      close();
  ...
  return <div className="draft-editor">
    ...
    <div className="contribution-purpose" aria-label="工作记录与成果草稿的区别"><div><b>Agent 工作记录</b><span>本地过程资料</span></div><span className="purpose-arrow" aria-hidden="true">→</span><div><b>成果草稿</b><span>待审阅的分享内容</span></div></div>
    ...
    {draft.body && <section className="contribution-result" aria-label="AI 整理结果">
      ...
      <h2>{draft.title}</h2>
      <p className="repository-reference">仓库链接：{repoUrl ? <a href={repoUrl} onClick={e => { e.preventDefault(); void run(() => api.call('open.link', repoUrl)); }}>{repoUrl}</a> : <span className="muted">未提供</span>}</p>
    ...
    <label className="field contribution-supplement">补充说明（可选）<textarea rows={3} aria-label="补充说明（可选）" placeholder="补充背景、注意事项或更正说明" disabled={locked} value={value.supplement} onChange={e => change({ supplement: e.target.value })}/></label>
