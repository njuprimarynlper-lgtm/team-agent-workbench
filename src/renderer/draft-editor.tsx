...
export function DraftEditor({ draft, session, sourceTitle, sourceSession, transfers, run, notice, close, viewShared }: { draft: Draft; session?: AgentSession; sourceTitle?: string; sourceSession?: AgentSession; transfers: Transfer[]; run: <T>(fn: () => Promise<T>) => Promise<T | undefined>; notice: (s: string) => void; close: () => void; viewShared: (projectId: string, path: string) => void }) {
  ...
  const generating = draft.generation === 'running', ready = draft.generation === 'ready', isMerge = !!draft.mergeSources?.length;
  ...
  const mergeEditor = useAutosave('content-merge:' + draft.id + ':' + (draft.generationFinishedAt || 'pending'), { title: draft.title, body: draft.body }, value => api.call('content.merge.save', { id: draft.id, ...value }));
  ...
  const back = () => void run(async () => {
    setLeaving(true);
    try {
      await (isMerge ? mergeEditor.flush() : editor.flush());
      close();
  ...
  const backLabel = isMerge ? '返回项目文档' : sourceTitle ? `返回“${sourceTitle}”` : '返回原会话';
  ...
  const status = draft.mergeCompletedAt ? '语义合并已保存' : generating ? session?.approvals.length ? '需要你确认一项操作' : isMerge ? '正在进行语义融合…' : '正在整理…' : ready ? isMerge ? '融合完成，请审阅后确认' : '已整理好，请确认后上传' : draft.generation === 'error' ? isMerge ? '语义融合失败，可重试' : '整理失败，可重试' : '已停止，可重新整理';
  return <div className="draft-editor">
    ...
    <section className="preparation-summary" aria-label="成果整理进度">
      <div className="preparation-status" aria-live="polite" aria-label="整理状态">{generating && <LoaderCircle size={17} className="spin"/>}<strong>{draft.mergeCompletedAt ? status : draft.submitted ? draftStatus(draft, transfer) : status}</strong>{generating && <span className="muted small" aria-label="整理已用时间">{elapsed < 60 ? `${elapsed} 秒` : `${Math.floor(elapsed / 60)} 分 ${elapsed % 60} 秒`}</span>}{!locked && !generating && !draft.submitted && <button className="text-button" onClick={() => void run(() => { notice(''); return api.call('draft.retry', { id: draft.id }); })}>{draft.generation === 'error' ? '重试' : isMerge ? '重新进行语义融合' : hasNewSource ? '重新整理并包含新内容' : '重新整理'}</button>}</div>
      <p className="muted small preparation-source">{isMerge ? `融合 ${draft.mergeSources!.length} 条项目文档；仅借用“${sourceTitle || '工作会话'}”的模型环境，不会写入原对话` : <>来自“{sourceTitle || '原工作会话'}”{draft.snapshot ? `，采用截至 ${new Date(draft.snapshot.capturedAt).toLocaleString()} 的内容` : ''}{hasNewSource ? '；本次结果不包含原会话新增内容' : ''}</>}</p>
    ...
    {isMerge && draft.body && <section className="semantic-merge-review" aria-label="语义合并结果"><div className="merge-source-list"><b>本次融合来源</b>{draft.mergeSources!.map(source => <span key={source.id}>{source.title} · {source.author} · v{source.revision}</span>)}</div><label className="field">合并后标题<input aria-label="合并后标题" disabled={locked} value={mergeEditor.value.title} onChange={e => mergeEditor.change({ ...mergeEditor.value, title: e.target.value })}/></label><label className="field">融合后的项目文档<textarea aria-label="融合后的项目文档" rows={20} disabled={locked} value={mergeEditor.value.body} onChange={e => mergeEditor.change({ ...mergeEditor.value, body: e.target.value })}/></label>{!locked && mergeEditor.status !== '已保存' && <div className="row small muted" role="status"><span>{mergeEditor.status}</span>{mergeEditor.status.startsWith('保存失败') && <button className="text-button" onClick={mergeEditor.retry}>重试保存</button>}</div>}<details className="merge-safety"><summary>合并保存规则</summary><p>确认时会再次核对所有来源版本；任何来源已变化都会停止合并。只有保存成功后原条目才会归档，AI 无权直接删除或覆盖。</p></details></section>}
    ...
    </section> : !isMerge && draft.body && <section className="contribution-result" aria-label="整理结果">
    ...
    {!isMerge && <label className="field contribution-supplement">给团队的补充（可选）<textarea rows={3} aria-label="给团队的补充（可选）" placeholder="只有需要补充或更正时填写" disabled={locked} value={value.supplement} onChange={e => change({ supplement: e.target.value })}/></label>}
    {!isMerge && !locked && editor.status !== '已保存' && <div className="row small muted" role="status"><span>{editor.status}</span>{editor.status.startsWith('保存失败') && <button className="text-button" onClick={editor.retry}>重试保存</button>}</div>}
    {!isMerge && !artifacts.length && <div className="upload-destination" aria-label="上传位置" title={ready ? draft.target : undefined}><span>上传位置</span>{draft.binding ? <b>{draft.binding.project.name} / {destinationLabel}</b> : <b>未绑定项目，无法上传</b>}</div>}
    {isMerge ? <div className="draft-actions"><button className="secondary back-action" disabled={leaving} onClick={back}>{backLabel}</button><span className="spacer"/>{draft.mergeCompletedAt && draft.mergeProjectId && draft.mergeResultPath ? <button className="primary" onClick={() => viewShared(draft.mergeProjectId!, draft.mergeResultPath!)}>查看合并结果</button> : <button className="primary" disabled={busy || leaving || !ready || !mergeEditor.value.title.trim() || !mergeEditor.value.body.trim()} onClick={() => void run(async () => { setBusy(true); try { await mergeEditor.flush(); const result = await api.call<{ path: string }>('content.merge.commit', { id: draft.id }); setSubmitted(true); notice('语义合并已保存，原条目已归档'); viewShared(draft.mergeProjectId!, result.path); } finally { setBusy(false); } })}><Check size={15}/>{busy ? '正在核对并保存…' : `确认合并并归档 ${draft.mergeSources!.length} 条原文`}</button>}</div> : <div className="draft-actions">{!draft.submitted && !submitted ? <>{draft.body && <button className="secondary back-action" disabled={leaving} onClick={back}>{backLabel}</button>}<span className="spacer"/><button className="primary" disabled={busy || leaving || !ready || !(artifacts.length ? selectedArtifacts.length : draft.body.trim()) || !draft.binding} onClick={() => void run(async () => { setBusy(true); try { await editor.flush(); await api.call('draft.submit', { id: draft.id }); setSubmitted(true); notice(`已开始上传 ${artifacts.length ? selectedArtifacts.length : 1} 项成果；上传完成后可逐项查看结果`); } finally { setBusy(false); } })}><Upload size={15}/>{busy ? '正在上传…' : artifacts.length ? `确认上传 ${selectedArtifacts.length} 项` : '确认上传'}</button></> : <><button className="secondary back-action" onClick={back}>{backLabel}</button><span className="green row"><Check size={17}/>{transfers.some(item => item.status === 'error') ? '部分上传失败，可在对应成果中重试' : transfers.length && transfers.every(item => item.status === 'done') ? '已上传到公共区，可选择上方结果直接查看' : '正在上传'}</span>{!artifacts.length && transfer?.status === 'done' && <button className="primary compact" onClick={() => viewShared(transfer.binding.project.id, transfer.target)}>查看上传结果</button>}</>}</div>}
  </div>;
}
