...
import { contentAliasKey, contributionCategories, materialCategories, contributionCategoryInfo, titleSubject, type ContributionCategory } from '../shared/content';
...
function PreparationOptionsModal({ session, baseline, close, started }: { session: AgentSession; baseline?: PreparationCheckpoint; close: () => void; started: (categories: ContributionCategory[], scope: PreparationScope) => Promise<void> }) {
  ...
  const selected: ContributionCategory[] = [...materialCategories];
  const [busy, setBusy] = useState(false), [error, setError] = useState('');
  const [scope, setScope] = useState<PreparationScope>(canIncremental ? 'incremental' : 'full');
  return <Modal title={baseline ? '选择整理范围' : '整理成果'} close={close}><div className="modal-body">{baseline && <><p className="muted small">按 Session 的整理进度新建任务，删除记录或成果不会重置进度。</p><div className="preparation-options preparation-scope-options"><label className="preparation-option"><input type="radio" name="preparation-scope" aria-label="增量整理" checked={scope === 'incremental'} disabled={busy || !canIncremental} onChange={() => setScope('incremental')}/><span><b>增量整理</b><small>{canIncremental ? `只整理最近成功整理后新增或续写的 ${incrementalCount} 条消息。` : delta.reason}</small></span></label><label className="preparation-option"><input type="radio" name="preparation-scope" aria-label="全量整理" checked={scope === 'full'} disabled={busy} onChange={() => setScope('full')}/><span><b>全量整理</b><small>重新读取完整 Session；需要重新提炼已删除成果时选此项。</small></span></label></div></>}<p className="muted small">只保留值得复用的新内容，最多 3 项；没有新内容就不生成。类别会自动判断。</p>{error && <div className="inline-error" role="alert">{error}</div>}</div><footer><button className="secondary" disabled={busy} onClick={close}>取消</button><button className="primary" disabled={busy || !selected.length || (scope === 'incremental' && !canIncremental)} onClick={async () => { setBusy(true); setError(''); try { await started(selected, scope); close(); } catch (e: any) { setError(e.message); } finally { setBusy(false); } }}>{busy ? '正在启动…' : baseline ? `开始${scope === 'incremental' ? '增量' : '全量'}整理` : '开始整理'}</button></footer></Modal>;
}
...
function App() {
  ...
  const myPendingTasks = assignments.filter(task => task.assignee === connection?.profile.username && ['assigned', 'in_progress'].includes(task.status));
  ...
  const composer = currentInput.text, selectedSources = currentInput.sourceIds, answers = currentInput.answers, selectedCapabilities = currentInput.capabilities || [];
  ...
  const viewShared = (targetProjectId: string, targetPath: string) => { setActivityResult(undefined); setProjectId(targetProjectId); setLibraryFocus({ projectId: targetProjectId, path: targetPath }); setView('library'); };
  const viewActivityResult = (item: ContentUpdate) => { setProjectId(item.projectId); setLibraryFocus(undefined); setActivityResult(item); setView('library'); };
  const openLibrary = () => { setActivityResult(undefined); setLibraryFocus(undefined); setView('library'); };
  ...
  return <div className="app">
    <nav className="rail">
      ...
      <button className={'rail-feature ' + (view === 'assignments' ? 'active' : '')} title={projectAdmin ? '项目派活' : '我的任务'} aria-label={projectAdmin ? '项目派活' : '我的任务'} disabled={!project} onClick={() => { setView('assignments'); void refreshAssignments(); }}><ClipboardList size={22}/><span>任务</span>{myPendingTasks.length > 0 && <b>{Math.min(myPendingTasks.length, 99)}</b>}</button>
      <button className={'rail-feature ' + (view === 'updates' ? 'active' : '')} title="团队动态" data-tooltip="团队动态" aria-label="团队动态" onClick={() => setView('updates')}><BellRing size={22}/><span>动态</span>{unreadContentUpdates.length > 0 && <b aria-label={`${unreadContentUpdates.length} 条未读团队动态`}>{Math.min(unreadContentUpdates.length, 99)}</b>}</button>
      ...
      <button className={view === 'conclusions' ? 'active' : ''} title="本地项目成果库" data-tooltip="本地项目成果库" aria-label="本地项目成果库" disabled={!project} onClick={() => setView('conclusions')}><FileText size={22}/></button>
      ...
      <button title="团队项目成果库" data-tooltip="团队项目成果库" aria-label="团队项目成果库" disabled={!project || !connection?.connected} className={view === 'library' ? 'active' : ''} onClick={openLibrary}><Search size={22}/></button>
    ...
    {view === 'sessions' && project && myPendingTasks.length > 0 && <div className="callout assignment-welcome"><ClipboardList size={20}/><div><b>你有 {myPendingTasks.length} 项待办任务</b><small>{myPendingTasks[0].title} · 查看任务要求和关联成果后开始工作。</small></div><button className="primary compact" onClick={() => setView('assignments')}>查看我的任务</button></div>}
    ...
    {view === 'updates' && <ContentUpdatesPanel
      updates={contentUpdates}
      aliases={state.settings.contentAliases || {}}
      view={viewActivityResult}
      deletionResolved={async () => { setContentUpdates(await api.call<ContentUpdate[]>('content.updates')); await refresh(); }}
      apply={async item => {
        const result = await api.call<ConclusionOrganization>('conclusion.import', { projectId: item.projectId, contentId: item.id, expectedRevision: item.revision });
        if (result) {
          setContentUpdates(await api.call<ContentUpdate[]>('content.updates'));
          const label = ({ created: '已存入本地成果库', updated: '已更新项目成果来源', duplicate: '这条成果已在“本地项目成果库”中' } as const)[result.action];
          setNotice(label + `：“${conclusionTitle(result.conclusion)}”`);
        }
        return result;
      }}
    />}
    {view === 'conclusions' && project && <ConclusionLibrary refreshToken={state.accountSync?.syncedAt} key={project.id} project={project} sessions={mergeSessions} notice={setNotice}
    ...
    {view === 'library' && project && <SharedContentLibrary key={project.id + ':' + (activityResult?.projectId === project.id ? activityResult.eventId : 'all')} project={project} username={connection?.profile.username || ''} admin={projectAdmin} aliases={state.settings.contentAliases || {}} aliasSaved={refresh} notice={setNotice} mergeSessions={mergeSessions}
      ...
      attach={async (item, sessionIds) => {
        ...
        setNotice(`已将“${displayTitle}”加入会话：${names.join('、')}；同时保存到“本地项目成果库”`);
      }}
    />}
    ...
    {preparingSession && <PreparationOptionsModal session={preparingSession} baseline={preparationCheckpoint(preparingSession, state.drafts)} close={() => setPreparingSession(undefined)} started={async (categories, scope) => { const d = await api.call<Draft>('draft.prepare', { id: preparingSession.id, scope, categories }); setDraftReturnToList(false); setDraftId(d.id); setView('drafts'); await refresh(); }}/>}
    ...
    {settingsOpen && (
      <SettingsModal settings={state.settings} providers={state.providers} auth={state.auth} project={project} projectAdmin={projectAdmin} close={() => setSettingsOpen(false)} run={run} projectSaved={async revision => { await refresh(); setNotice(`项目设置已保存为 v${revision}，“项目说明.md”已同步更新`); }}/>
    )}
...
function ConnectModal({ settings, connection, egress, close, run, onConnected }: { settings: Settings; connection: Snapshot['connection']; egress: Snapshot['egress']; close: () => void; run: Run; onConnected: () => void }) {
  ...
  return <Modal title="登录团队工作台" close={close} wide><div className="modal-body">
    ...
    <div className="form-grid">
      <label className="field full">代码目录（选填）<div className="row"><input aria-label="代码目录（选填）" value={localPath} onChange={e => setLocalPath(e.target.value)}/><button className="secondary" onClick={() => choose(setLocalPath)}>选择目录</button></div></label>
...
function NewSessionModal({ settings, auth, projects, projectId, assignment, close, run, created }: { assignment?: ProjectAssignment; settings: Settings; auth: Snapshot['auth']; projects: Project[]; projectId: string; close: () => void; run: Run; created: (s: AgentSession) => void }) {
  ...
  useEffect(() => { void api.call<string>('workspace.research').then(setResearchCwd); }, []);
  ...
  return <Modal title="新建工作会话" close={close} wide><div className="modal-body">{assignment && <div className="callout"><div><b>开始任务：{assignment.title}</b><small>将带入任务说明和 {assignment.references.length} 条关联成果。创建后可检查输入草稿，再发送给 AI。</small></div></div>}
    ...
    <div className="new-session-grid"><div>
      <ProviderAuthPanel key={selected} provider={selected} auth={auth[selected]} cwd={authCwd}/>
      <label className="field">代码目录（选填）<div className="row"><input aria-label="代码目录（选填）" placeholder="留空用于调研，文件会保存在此会话的独立目录" value={cwd} onChange={e => setCwd(e.target.value)}/><button className="secondary" title="选择代码目录（选填）" onClick={() => void run(async () => { const dir = await api.call<string>('choose.directory'); if (dir) setCwd(dir); })}><FolderOpen size={17}/></button></div></label>
      <label className="field">绑定共享项目<select aria-label="绑定共享项目" disabled={!!assignment} value={project} onChange={e => { setProject(e.target.value); const directory = Object.entries(settings.projectDirectories || {}).find(([key]) => key.endsWith(':' + e.target.value))?.[1]; if (directory) setCwd(directory); }}><option value="" disabled>请选择工作组下的项目</option>{[...new Set(projects.map(p => p.groupName || ''))].map(g => <optgroup key={g} label={projects.find(p => (p.groupName || '') === g)?.groupLabel || '项目'}>{projects.filter(p => (p.groupName || '') === g).map(p => <option value={p.id} key={p.id}>{p.name}</option>)}</optgroup>)}</select></label>
    ...
    </div><ModelPicker key={selected + authCwd} provider={selected} cwd={authCwd} ready={!!canUseProvider(auth[selected]) && auth[selected].cwd === authCwd} model={model} changed={setModel}/></div>
    <PermissionPicker key={selected + authCwd} provider={selected} cwd={authCwd} mode={permissionMode} changed={setPermissionMode}/>
  </div><footer><button className="secondary" onClick={close}>取消</button><button className="primary" disabled={!authCwd || !project || busy || !canUseProvider(auth[selected]) || auth[selected].cwd !== authCwd} onClick={() => void run(async () => { setBusy(true); try { const s = await api.call<AgentSession>(assignment ? 'assignment.start' : 'session.create', { provider: selected, cwd: authCwd, model: model || undefined, permissionMode, includeBrief, projectId: project || undefined, ...(assignment ? { taskId: assignment.id, revision: assignment.revision } : {}) }); created(s); } finally { setBusy(false); } })}>创建会话</button></footer></Modal>;
}
