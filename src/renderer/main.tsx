...
import './cli-connection-notice.css';
...
function AccountWorkspace({ state, refresh }: { state: Snapshot; refresh: () => Promise<void> }) {
  ...
  return <div className="app">
    ...
    <div className="shell"><header className="topbar"><div><strong>团队工作台 <span className="version">用户版</span></strong></div><div className="topbar-actions"><AccountSyncStatus state={state.accountSync} connected={!!connection?.connected}/><button className="secondary compact" title="打开登录窗口；同一账号共用本机资料，不同账号相互隔离" onClick={() => void run(async () => { await api.call('window.new'); })}><Plus size={14}/>新建账号窗口</button><button className={'connection-button ' + (connection?.connected ? 'online' : '')} onClick={() => setConnectOpen(true)}><span className="dot"/>{connection?.connected ? `${connection.profile.mode === 'local' ? '本地模拟 · ' : ''}${connection.profile.name} · ${connection.profile.username}` : '连接团队账号'}<ChevronRight size={14}/></button></div></header>
...
function ConnectModal({ settings, connection, egress, close, run, onConnected }: { settings: Settings; connection: Snapshot['connection']; egress: Snapshot['egress']; close: () => void; run: Run; onConnected: () => void }) {
  ...
  const [viaSharedServer, setViaSharedServer] = useState(!!settings.egress?.viaSharedServer);
  ...
  return <Modal title="登录团队工作台" close={close} wide><div className="modal-body">
    ...
    <section className="member-egress-option"><label className="check-row"><input type="checkbox" checked={egressEnabled} onChange={e => setEgressEnabled(e.target.checked)}/><Network size={16}/><span><b>通过管理端访问模型服务</b><small>只有本机不能直接访问外网时才需要开启；仍使用你的个人 CLI 账号。</small></span></label>{egressEnabled && <><EgressRouteChoice value={viaSharedServer} onChange={setViaSharedServer} disabled={busy || local}/><label className="field">管理端接入码<textarea aria-label="管理端网络出口接入码" rows={3} placeholder={egress?.hasAccessCode ? '接入码已保存；如管理端更新过接入码，再粘贴新的内容' : '粘贴管理端“网络出口”页面复制的接入码'} value={inviteCode} onChange={e => { setInviteCode(e.target.value); if (e.target.value.trim().startsWith('TAE2.')) setViaSharedServer(true); }}/></label><p className={'egress-inline-status ' + (egress?.available ? 'online' : '')}>{egress?.available ? '管理端网络出口可用' : egress?.configured ? '已保存接入信息，登录后会自动检测' : '尚未配置接入信息'}</p></>}</section>
  </div><footer>{connection?.connected && <button className="secondary" onClick={() => void run(async () => { await api.call('remote.disconnect'); close(); })}><Unplug size={14}/>断开连接</button>}<span className="spacer"/><button className="secondary" onClick={close}>取消</button><button className="primary" disabled={busy || !profile.host || !profile.username || !password || (local && !profile.localRoot?.trim()) || (egressEnabled && !inviteCode.trim() && !egress?.hasAccessCode)} onClick={async () => { setBusy(true); setError(''); try { await api.call('remote.connect', { profile: { ...profile, fingerprint: effectiveFingerprint, projects: [], workPath: '', manifestPath: '' }, password, egress: { enabled: egressEnabled, viaSharedServer: !local && viaSharedServer, inviteCode: inviteCode.trim() || undefined } }); setPassword(''); setInviteCode(''); onConnected(); } catch (e: any) { setError(e.message); } finally { setBusy(false); } }}>{busy ? '正在登录…' : '登录'}</button></footer></Modal>;
}
