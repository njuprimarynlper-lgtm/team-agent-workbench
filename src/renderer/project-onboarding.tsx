...
export function ProjectOnboarding({ profile, workspace, close, created }: { profile: ConnectionProfile; workspace: WorkspaceAccess; close: () => void; created: (p: Project) => Promise<void> }) {
  ...
  const unavailable = !workspace.canCreateProject || !!workspace.accessError || !workspace.isEmpty;
  return <div className="modal-backdrop"><section className="modal wide project-onboarding" role="dialog" aria-label="完善项目资料"><header><h2>建立第一个项目</h2><button className="icon" aria-label="稍后填写项目资料" disabled={busy} onClick={close}><X size={19}/></button></header>
    <div className="modal-body"><div className="callout"><div><b>{workspace.groupLabel || workspace.groupName} · 项目初始化</b><small>{unavailable ? '工作组已有内容或权限已变化，请关闭引导后查看最新工作组状态。填写内容仍保留在本机。' : '这个工作组还是空的，你是本组的项目子管理员。先说明团队要解决什么问题、做到什么程度，成员便能围绕同一目标开展工作。'}</small><small>账号：{profile.username} · 共享区：{profile.name} · {workspace.canonicalPath}</small></div></div>
      <p className="muted small">带 * 的内容必填；暂未确定的事项可注明待确认。填写内容先在本机暂存，点击保存后才写入共享项目。也可以稍后填写，继续本地工作。</p>
