...
export function SharedContentLibrary({ project, username, admin, aliases, aliasSaved, attach, attachSessions, notice, mergeSessions, mergeStarted, focusPath, focusHandled, resultId, returnToUpdates, activity, activityChanged, embedded = false }: { embedded?: boolean; project: Project; username: string; admin: boolean; aliases: Record<string, string>; aliasSaved: () => Promise<void>; attach: (item: SharedContent, sessionIds: string[]) => Promise<void>; attachSessions: AgentSession[]; notice: (text: string) => void; mergeSessions: AgentSession[]; mergeStarted: (draft: Draft) => void; focusPath?: string; focusHandled?: () => void; resultId?: string; returnToUpdates?: () => void; activity?: ContentUpdate; activityChanged?: () => Promise<void> }) {
  ...
  const load = async (resetSelection = true) => {
    const scope = contentScope.current, sequence = ++loadSequence.current, localSequence = ++personalSequence.current; setBusy(true); if (resetSelection) setDeleteSelection([]);
    try {
    ...
    finally { if (scope === contentScope.current && sequence === loadSequence.current) setBusy(false); }
  };
  ...
  const storePersonal = async (value: SharedContent) => {
    ...
    try {
      ...
      setSelected(''); const refreshed = await load();
      if (scope !== contentScope.current) return;
  ...
  useEffect(() => { if (!focusPath) return; const target = items.find(value => value.path === focusPath); if (target) { setLabelFilter('all'); setSearch(''); setKind('all'); setMergeSelection([]); open(target); focusHandled?.(); } }, [focusPath, items]);
  ...
  const visibleItems = admin && showAll ? items : items.filter(value => differences[value.id]);
  ...
  return <div className={(embedded ? 'results-library-pane' : 'workspace-page') + ' team-results' + (resultId ? ' activity-result-page' : '')}>
    ...
    <div className={'content-library' + (resultId ? ' single-content-result' : '')}>{!resultId && <div className="content-cards">{filtered.map(i => <article className={'content-card ' + (i.id === selected ? 'selected' : '') + (mergeSelection.includes(i.id) || deleteSelection.includes(i.id) ? ' merge-selected' : '')} key={i.id}>{selectingDelete && <label className="content-card-select check-row"><input type="checkbox" aria-label={`选择删除团队成果：${displayTitle(i)}`} checked={deletion.selected.some(value => value.id === i.id)} disabled={busy || !canDeleteSharedContent(i, username, admin) || !deleteSelection.includes(i.id) && deletion.selected.length >= 100} onChange={event => setDeleteSelection(current => event.target.checked ? [...new Set([...current, i.id])] : current.filter(id => id !== i.id))}/><span>{canDeleteSharedContent(i, username, admin) ? '选择删除' : '无删除权限'}</span></label>}{selectingMerge && i.kind === 'contribution' && <label className="content-card-select check-row"><input type="checkbox" aria-label={`选择合并：${i.title}`} checked={mergeSelection.includes(i.id)} disabled={busy} onChange={e => toggleMerge(i.id, e.target.checked)}/><span>加入语义合并</span></label>}<button className="content-card-summary" disabled={editing} onClick={() => open(i)}><span className="content-category-badge">{categoryLabel(i)}</span><small className="team-result-difference">{differences[i.id] ? teamResultDifferenceLabels[differences[i.id]!] : '个人库已有此版本'}</small><b>{displayTitle(i)}</b>{alias(i) && <small>本地别名 · 远端标题：{titleSubject(i.title) || i.title}</small>}<small>{i.author} · v{i.revision}</small>{i.sourceSessionTitle && <small>来源会话：{i.sourceSessionTitle}</small>}<small>{new Date(i.updatedAt).toLocaleString()}</small><p>{resultPreview(i.description) || '共享文件'}</p>{!!i.attachments?.length && <small>{i.attachments.length} 个附件</small>}</button>{!selectingMerge && !selectingDelete && <div className="content-card-actions"><button className="primary compact" aria-label={`存入个人成果库：${displayTitle(i)}`} disabled={editing || busy || !!savingId || !differences[i.id]} onClick={() => void storePersonal(i)}>{savingId === i.id ? '正在存入…' : differences[i.id] ? '存入个人成果库' : '个人库已有'}</button>{admin && <><button className="secondary compact danger" aria-label={`从共享区移除：${displayTitle(i)}`} disabled={editing || busy} onClick={() => setPendingDelete(i)}>从共享区移除</button><button className="primary compact" disabled={editing || busy} onClick={() => edit(i)}>编辑成果</button></>}</div>}</article>)}{!filtered.length && <p className="muted">{busy ? '正在比对团队与个人成果…' : !showAll && !visibleItems.length && items.length ? '团队成果与个人库已一致，暂无需要存入的内容。' : hasFilters ? '暂无匹配成果，可清除筛选查看。' : '暂无团队成果'}</p>}</div>}
      <section className="content-detail">{item ? <><div className="content-detail-title"><span className="content-category-badge">{categoryLabel(item)}</span><h2>{displayTitle(item)}</h2></div>{alias(item) && <p className="muted small">本地别名，仅你可见 · 远端标题：{titleSubject(item.title) || item.title}</p>}<p className="muted small">提交人：{item.author} · 最近维护：{item.updatedBy} · v{item.revision}{item.sourceSessionTitle ? ` · 来源会话：${item.sourceSessionTitle}` : ''}</p>{editing ? <><label className="field">标题<input aria-label="团队成果标题" value={title} onChange={e => setTitle(e.target.value)}/><small>标题开头的【标签】用于筛选。</small></label><label className="field">内容 / 说明<textarea aria-label="团队成果内容" rows={14} value={body} onChange={e => setBody(e.target.value)}/></label><label className="field">仓库链接（可选）<input value={repo} onChange={e => setRepo(e.target.value)}/></label><div className="row"><button className="secondary" disabled={busy} onClick={() => open(item)}>取消编辑</button>{item.kind !== 'contribution' && <button className="secondary" disabled={busy || !title.trim()} onClick={async () => { setBusy(true); try { const saved = await window.workbench.call('content.replace', { projectId: project.id, change: { id: item.id, revision: item.revision, action: 'save', title, description: body, curate: admin } }); if (saved) { setEditing(false); await load(); notice('文件已替换为新修订'); } } catch (e: any) { setError(e.message); } finally { setBusy(false); } }}>选择文件并保存替换</button>}<button className="primary" disabled={busy || !title.trim()} onClick={() => void save('save')}>保存修改</button></div></> : <><div className="markdown"><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ a: ({ href, children }) => <a href={href} onClick={e => { e.preventDefault(); if (href && /^https?:/.test(href)) void window.workbench.call('open.link', href); }}>{children}</a>, img: ({ alt }) => <span>[图片：{alt}]</span> }}>{item.description || '此项为共享文件。'}</ReactMarkdown></div><details className="content-provenance"><summary>来源详情</summary><p>来自：{item.sourceSessionTitle || '手工提交'} · 提交人：{item.author} · 共享给：{project.name}项目组</p>{item.sourceDetails && <pre style={{ whiteSpace: 'pre-wrap' }}>{item.sourceDetails}</pre>}</details><SharedAttachments key={item.id} item={item} projectId={project.id}/>{item.provenance?.length ? <details className="content-provenance"><summary>查看融合来源（{item.provenance.length}）</summary>{item.provenance.map(source => <p key={source.id + ':' + source.revision}>{source.title} · {source.author} · v{source.revision} · {new Date(source.updatedAt).toLocaleString()}</p>)}</details> : null}{item.repoUrl && <p>仓库：{item.repoUrl}</p>}{item.git && <p className="muted small">分支 {item.git.branch} · Commit {item.git.commit || '无'} · {item.git.dirty ? '存在未提交改动' : '无未提交改动'}</p>}<div className="row">{!activity && <button className="primary" disabled={busy || !!savingId || !differences[item.id]} onClick={() => void storePersonal(item)}>{savingId === item.id ? '正在存入…' : differences[item.id] ? '存入个人成果库' : '个人库已有此版本'}</button>}{item.kind !== 'contribution' && <button className="secondary" onClick={() => void window.workbench.call('remote.download', { projectId: project.id, path: item.path }).catch(e => setError(e.message))}>下载</button>}<button className="secondary" onClick={() => { setAliasItem(item); setAliasValue(alias(item)); }}>设置本地别名</button><button className="secondary" disabled={!attachSessions.length} title={attachSessions.length ? '选择一个或多个会话作为参考资料' : '请先为此项目创建工作会话'} onClick={() => { setAttachItem(item); setAttachSelection([]); }}>加入会话</button>{writable && (resultId && !admin ? <details className="activity-own-actions"><summary>管理我的提交</summary><div className="row"><button className="secondary" disabled={busy} onClick={() => edit(item)}>修改自己的提交</button><button className="secondary danger" disabled={busy} onClick={() => setPendingDelete(item)}>从共享区移除</button></div></details> : <><button className="primary" onClick={() => edit(item)}>{admin ? '编辑成果' : '修改自己的提交'}</button><button className="secondary danger" disabled={busy} onClick={() => setPendingDelete(item)}>从共享区移除</button></>)}</div></>}</> : <p className="muted">{resultId ? (busy ? '正在读取这条成果…' : error ? '读取失败，请重试。' : '这条成果已删除或被合并，当前结果已不可用。已保存的本地成果仍会保留。') : '选择一条成果查看内容'}</p>}</section></div>
