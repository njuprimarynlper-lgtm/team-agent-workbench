...
export interface Transfer { conclusionSourceId?: string; dependsOn?: string[]; attachment?: boolean; metadata?: ContentMetadata; trajectoryHash?: string; cacheCleared?: boolean; sha256?: string; completedAt?: string; id: string; kind: 'upload' | 'history' | 'download'; name: string; status: 'queued' | 'running' | 'done' | 'error'; bytes: number; total: number; target: string; projectName: string; createdAt: string; error?: string; sessionId?: string; localPath: string; binding: RemoteBinding }
export interface DraftDestination { id: string; path: string; description: string }
export interface DraftArtifact { topic?: string; evidenceIds?: string[]; sourceDetails?: string; attachments?: { fileId: string; selected: boolean }[]; id: string; category: ContributionCategory; title: string; titleAlias?: string; fields: Record<string, string>; body: string; repoUrl?: string; target: string; selected: boolean; submitted?: string }
...
export interface ContentUpdate { eventId: string; projectId: string; projectName: string; id: string; path?: string; removedPath?: string; title: string; author?: string; updatedBy?: string; revision: number; category?: ContributionCategory; sourceSessionTitle?: string; change: 'new' | 'updated' | 'deleted' | 'merged'; sourceTitles?: string[]; occurredAt: string; detectedAt: string; readAt?: string; statusChangedAt?: string; actions?: ContentUpdateAction[]; archiveReason?: 'own_change'; unavailableAt?: string }
export interface ConclusionSource { publication?: { path: string; sha256: string; revision: number; contentId?: string }; details?: string; sha256?: string; id: string; kind: 'remote' | 'session' | 'manual' | 'conclusion'; title: string; content?: string; revision?: number; path?: string; updatedAt: string }
