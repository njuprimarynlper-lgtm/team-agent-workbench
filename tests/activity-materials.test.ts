...
test('activity history distinguishes the original revision from the actually imported result without opening windows', async () => {
  ...
  assert.match(modal, /旧版风险标题/); assert.match(modal, /更新后的综合结论/); assert.match(modal, /将第 2 版加入项目资料/);
  ...
  assert.match(repaired, /已纠正旧版自动归并/); assert.match(repaired, /已恢复为独立项目资料/); assert.doesNotMatch(repaired, /已加入更新后的成果/);
});
