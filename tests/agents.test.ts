...
for (const provider of ['codex', 'cursor'] as Provider[]) test(provider + ': native session mapping, streaming, approvals and completion', async () => {
  ...
  await fs.writeFile(launcher, process.platform === 'win32' ? `@echo off\r\nset TEST_PROVIDER=${provider}\r\n"${process.execPath}" "${script}"\r\n` : `#!/bin/sh\nTEST_PROVIDER=${provider} "${process.execPath}" "${script}"\n`, { mode: 0o755 });
