...
test('local account automatically discovers assigned groups, tolerates no membership, and never rebinds existing sessions', async () => {
  ...
  try {
    ...
    assert.equal(member.workspaceReady, true); assert.equal(member.remote.workspaces.length, 0); assert.equal(member.remote.profile!.projects.length, 0);
    assert.equal(member.remote.profile!.workPath, ''); assert.equal((await member.createSession('codex', root)).binding, undefined);
    ...
    await member.remote.loadManifest();
